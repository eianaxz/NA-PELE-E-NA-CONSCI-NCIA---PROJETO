...
class MenuApp:
    def __init__(self, root, sistema_cadastro, user_data):
        ...
        self.menu_frame = tk.Frame(
            self.root, width=200, height=600, bg="#100720", bd=2, relief="raised")
        self.menu_frame.pack(side=tk.LEFT, fill=tk.Y, padx=10, pady=10)

        # Frame para conteúdo
        self.content_frame = tk.Frame(
            self.root, bg="#100720", bd=2, relief="groove")
    ...
    def setup_menu(self):
        ...
        style.map('Menu.TButton', background=[
                  ('active', '#e0e0e0')], foreground=[('active', 'black')])

        for text, command in buttons:
            btn = ttk.Button(self.menu_frame, text=text, width=25,
                             command=command, style='Menu.TButton')  # Aumentado o width
            # Adicionado fill=tk.X para preencher a largura
    ...
    def show_mundos_consciencia(self):
        ...
        style.map('Category.TButton', background=[
                  ('active', '#e0e0e0')], foreground=[('active', 'black')])
        ...
        for categoria, color in categorias:
            ...
            if "Dilemas éticos" in categoria:
                def btn_command(): return self.start_historia_elias()
            else:
                def btn_command(): return messagebox.showinfo(
                    "Em desenvolvimento", "Esta categoria estará disponível em breve!")
    ...
    def start_historia_elias(self):
        ...
        self.clear_content_frame()
        self.mundo_elias = MundoConscienciaElias(
            root=self.root, user_data=self.user_data, content_frame=self.content_frame, menu_app=self)
    ...
    def show_perfil_personalizado(self):
        ...
        if self.user_data and self.user_data.get("profile"):
            ...
            profile_description = profile_info.get(
    ...
    # Atualiza self.user_data com todos os dados do usuário
        self.user_data = {
            "id": user[0],
            "nome": user[1],
            "apelido": user[2],
    ...
    # Frame principal para os campos
        fields_frame = tk.Frame(self.content_frame, bg="#100720")
        fields_frame.pack(pady=20, padx=50, fill=tk.BOTH, expand=True)
    ...
    # Campos editáveis
        ...
        style.map('Edit.TButton', background=[
                  ('active', '#e0e0e0')], foreground=[('active', 'black')])

        for i, (label, field, value) in enumerate(fields):
            # Frame para cada campo
            field_frame = tk.Frame(fields_frame, bg="#100720", highlightbackground="#3D087B",
                                   highlightthickness=2, bd=0, relief="ridge")
            field_frame.pack(fill=tk.X, pady=10, ipady=5)

        # Label do campo
            tk.Label(field_frame, text=label, font=("Times New Roman", 13),
                     bg="#100720", fg="white", width=15).pack(side=tk.LEFT, padx=10)

        # Frame para entrada e botão
            entry_btn_frame = tk.Frame(field_frame, bg="#100720")
            entry_btn_frame.pack(side=tk.RIGHT, fill=tk.X,
                                 expand=True, padx=10)

        # Variável e entrada do campo
            self.entry_vars[field] = tk.StringVar(value=value)

            if field == "senha":
                entry = ttk.Entry(entry_btn_frame, textvariable=self.entry_vars[field],
                                  width=25, show="*")
                entry.pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=3)

            # Botão para mostrar/ocultar senha
                ttk.Button(entry_btn_frame, text="👁️", width=4,
                           command=lambda e=entry: self.toggle_password_visibility(e)).pack(side=tk.LEFT, padx=5)
            else:
                entry = ttk.Entry(entry_btn_frame, textvariable=self.entry_vars[field],
                                  width=25)
                entry.pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=3)
        ...
        # Botão de alteração
            ttk.Button(entry_btn_frame, text="Alterar", width=12, style='Edit.TButton',
                       command=lambda f=field: self.edit_field(f)).pack(side=tk.RIGHT, padx=5)
    ...
    def edit_field(self, field):
        ...
        if field == "nome":
            ...
            new_value = self.show_input_dialog(
                f"Alterar {field}", f"Novo {field}:", current_value)

            if new_value is not None:
                nome_sem_espacos = new_value.replace(" ", "")
                if not nome_sem_espacos.isalpha() or len(nome_sem_espacos) > 20:
                    messagebox.showerror(
                        "Erro", "Nome inválido. Insira seu novo nome novamente.")
                    return
            ...
            if new_value is not None:
                if " " in new_value or len(new_value) > 10:
                    messagebox.showerror(
                        "Erro", "Apelido inválido. Insira seu novo apelido novamente.")
                    return
            ...
            if new_value is not None:
                if not self.validar_email(new_value):
                    messagebox.showerror(
                        "Erro", "Email inválido. Insira seu novo email novamente.")
                    return
    ...
    def show_input_dialog(self, title, prompt, initial_value=""):
        ...
        tk.Label(dialog, text=prompt, bg="#100720",
                 fg="white", font=("Arial", 11)).pack(pady=15)
        ...
        ttk.Button(button_frame, text="OK", command=on_ok,
                   style='Menu.TButton').pack(side=tk.LEFT, padx=15)
        ttk.Button(button_frame, text="Cancelar", command=on_cancel,
                   style='Danger.TButton').pack(side=tk.RIGHT, padx=15)
    ...
    def update_user_field(self, field, new_value):
        ...
        try:
            ...
            messagebox.showinfo(
                "Sucesso", f"{field.capitalize()} atualizado com sucesso!")
        except sqlite3.Error as e:
            messagebox.showerror(
                "Erro", f"Falha ao atualizar {field}: {str(e)}")
        finally:
            conn.close()
    ...
    def change_password(self):
        ...
        tk.Label(new_pass_frame, text="Nova Senha:", bg="#100720",
                 fg="white", font=("Arial", 11)).pack(side=tk.LEFT)
        new_pass_var = tk.StringVar()
        new_pass_entry = ttk.Entry(
            new_pass_frame, textvariable=new_pass_var, show="*", width=30)
        ...
        tk.Label(confirm_frame, text="Confirmar Senha:", bg="#100720",
                 fg="white", font=("Arial", 11)).pack(side=tk.LEFT)
        confirm_pass_var = tk.StringVar()
        confirm_pass_entry = ttk.Entry(
            confirm_frame, textvariable=confirm_pass_var, show="*", width=30)
        ...
        def validate_and_proceed():
            ...
            # Removido .replace(" ", "") para validar espaços
            if len(nova_senha) != 6 or " " in nova_senha:
                messagebox.showerror(
            ...
            try:
                msg = MIMEText(
                    f"Seu código de verificação para alteração de senha é: {codigo}\n\nEste código é válido por 5 minutos.")
                ...
                with smtplib.SMTP(self.SMTP_SERVER, self.SMTP_PORT) as server:
                    ...
                    server.sendmail(self.EMAIL_SENDER,
                                    self.user_data['email'], msg.as_string())
                ...
                def verify_code():
                    if datetime.now() > expiration_time:
                        messagebox.showerror(
                            "Erro", "Código expirado. Por favor, inicie o processo novamente.")
                        dialog.destroy()
                        return
...
# Cria o botão Verificar
                ...
                messagebox.showerror(
                    "Erro", f"Falha ao enviar código: {str(e)}\nVerifique suas credenciais e conexão de internet.")
                dialog.destroy()

        ttk.Button(dialog, text="Continuar", command=validate_and_proceed,
    ...
    def confirmar_exclusao_conta(self):
        ...
        try:
            msg = MIMEText(
                f"Seu código de verificação para exclusão de conta é: {codigo}\n\nEste código é válido por 5 minutos.")
            ...
            ttk.Button(dialog, text="Verificar e Excluir",
                       command=verify_and_delete, style='Danger.TButton').pack(pady=15)
            self.root.wait_window(dialog)
    ...
    def show_ajuda(self):
        ...
        # Reduzido padx de 80 para 50 para ampliar a moldura
        help_frame.pack(pady=25, padx=50, fill=tk.X)
        ...
        tk.Label(help_frame, text=help_text, font=("Times New Roman", 14),
                 # Aumentado wraplength
                 bg="#100720", fg="white", justify=tk.LEFT, wraplength=550).pack(pady=20, padx=20)
        ...
        style.map('Help.TButton', background=[
                  ('active', '#e0e0e0')], foreground=[('active', 'black')])

        ttk.Button(self.content_frame, text="Ver Perguntas Frequentes (FAQ)",
                   command=lambda: messagebox.showinfo(
                       "FAQ", "Funcionalidade em desenvolvimento!"),
